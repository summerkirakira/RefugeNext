...
import 'package:refuge_next/src/widgets/ship_info_neo/game_item_detail_page.dart'
    show kAttachSubTypeCn, gameItemDisplayName;
import 'package:refuge_next/src/widgets/ship_info_neo/weapon_attachment_detail_page.dart';

/// 武器配件信息小卡片,样式仿 [VehicleWeaponInfoCard]:
/// 左侧 wiki 题图,右侧 名称 + 厂商 + 类型标签 + 尺寸/aUEC 价格。
/// 点击默认跳转到 [AttachmentDetailPage]。
class WeaponAttachmentInfoCard extends StatelessWidget {
  const WeaponAttachmentInfoCard({
    super.key,
    required this.item,
    this.onTap,
  });
  ...
  void _defaultTap(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            AttachmentDetailPage(initialItem: item, allowSwitch: false),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final sub = item.subType;
    final typeText = sub == null
        ? null
        : (kAttachSubTypeCn[sub] ?? item.subTypeLabel ?? sub);
    final auec = _auec();
    final tags = <String>[
      if (typeText != null && typeText.isNotEmpty) typeText,
  ...
  Widget _thumb(BuildContext context, String? url) {
    ...
    if (url == null) {
      return Container(
        ...
        child: Icon(Icons.build_circle_outlined, color: Colors.grey[400]),
      );
    }
