...
/// 载具武器(机炮)信息小卡片,样式仿 AI 舰船卡 [VehicleInfoCard]:
/// 左侧 wiki 题图,右侧 名称 + 厂商 + 类型/尺寸标签 + aUEC 价格。
/// 点击默认跳转到 [VehicleWeaponDetailPage]。
class VehicleWeaponInfoCard extends StatelessWidget {
  const VehicleWeaponInfoCard({
    super.key,
    required this.item,
    this.onTap,
  });
  ...
  void _defaultTap(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            VehicleWeaponDetailPage(initialItem: item, allowSwitch: false),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final type = item.vehicleWeapon?.type;
    final auec = _auec();
    final tags = <String>[
