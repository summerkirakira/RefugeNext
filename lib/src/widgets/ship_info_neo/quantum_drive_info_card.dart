...
/// 量子引擎信息小卡片,样式仿 [ShieldInfoCard]:
/// 左侧 wiki 题图,右侧 名称 + 厂商 + 类别标签 + 尺寸/aUEC 价格。
/// 点击默认跳转到 [QuantumDriveDetailPage]。
class QuantumDriveInfoCard extends StatelessWidget {
  const QuantumDriveInfoCard({
    super.key,
    required this.item,
    this.onTap,
  });
  ...
  void _defaultTap(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            QuantumDriveDetailPage(initialItem: item, allowSwitch: false),
      ),
    );
  }
  ...
  Widget _thumb(BuildContext context, String? url) {
    ...
    if (url == null) {
      return Container(
        ...
        child: Icon(Icons.bolt_outlined, color: Colors.grey[400]),
      );
    }
