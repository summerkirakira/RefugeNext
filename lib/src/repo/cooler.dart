...
/// 冷却器仓库（`/api/vehicle-items` 中 type=Cooler 的物品）。
///
/// 数据落盘于 `wiki/coolers/<版本码>.json`；元素为完整 [GameItem]
/// （含嵌套 `cooler` 字段及顶层 name/manufacturer/uuid 等元数据）。
/// 端点支持 `version` 参数，[refresh] 可指定历史版本补拉旧数据。
class CoolerRepo extends VersionedListRepo<GameItem> {
  static final CoolerRepo _instance = CoolerRepo._internal();

  CoolerRepo._internal()
      : super(
          dirName: 'wiki/coolers',
          versionPrefKey: 'app.cooler.selected_version',
          fromJsonElement: GameItem.fromJson,
          toJsonElement: (e) => e.toJson(),
        );

  factory CoolerRepo() => _instance;
  ...
  /// 从 Wiki API 分页拉取指定游戏版本的全量冷却器数据,写入本地并切换到该版本。
  ///
  ...
  Future<void> refresh({String? version}) async {
    ...
    while (page <= _maxPages) {
      final res = await api.listVehicleItems(
        ...
        filterLeftSquareBracketTypeRightSquareBracket: 'Cooler',
        version: version,
      );
      final body = res.data;
  ...
  /// 当前选中版本的全部冷却器;本地无数据返回空列表(不触发网络请求)。
  Future<List<GameItem>> getCoolers() => getItems();
