...
import 'package:refuge_next/src/widgets/ship_info_neo/game_item_detail_page.dart'
    show gameItemDisplayName, kItemTypeValueCn, kItemClassValueCn, kFireModeCn;
...
/// 把单件个人武器(FPS)[GameItem] 翻译成一段用于 RAG 的中文描述。
String buildPersonalWeaponRagContent(GameItem item) {
  ...
  final typeCn = typeParts.isEmpty ? '个人武器' : typeParts.join(' ');
  ...
  if (w?.capacity != null) base.add('容量 ${w!.capacity} 发');
  if (w?.fireMode != null && w!.fireMode!.isNotEmpty) {
    base.add('射击模式 ${kFireModeCn[w.fireMode] ?? w.fireMode}');
  }
  if (w?.rpm != null) base.add('射速 ${_fmt(w!.rpm)} RPM');
  if (w?.pelletsPerShot != null && w!.pelletsPerShot! > 1) {
    base.add('每发弹丸 ${w.pelletsPerShot}');
  }
  ...
  final d = w?.damage;
  final dmg = <String>[
    if (d?.dpsTotal != null) 'DPS ${_fmt(d!.dpsTotal)}',
    if (d?.alphaTotal != null) 'Alpha ${_fmt(d!.alphaTotal)}',
    if (d?.max != null) '最大 ${_fmt(d!.max)}/弹匣',
  ];
  if (dmg.isNotEmpty) {
    var s = '伤害：${dmg.join('、')}';
    final a = d?.alpha;
...
/// 单件个人武器的 RAG 记录。
Map<String, dynamic> buildPersonalWeaponRagRecord(GameItem item) {
  return {
    'key': item.slug,
    'name': gameItemDisplayName(item),
    'name_en': item.name,
    'class_name': item.className,
    'type': 'weapon_personal',
    'content': buildPersonalWeaponRagContent(item),
    'source': item.webUrl ??
        (item.slug == null
            ? null
...
/// 把当前版本的全部个人武器导出为单个 JSON 数组文件,返回文件。
Future<File> exportPersonalWeaponsRagJson({
  void Function(int done, int total)? onProgress,
}) async {
  final items = await PersonalWeaponRepo().getPersonalWeapons();
  ...
  for (var i = 0; i < items.length; i++) {
    records.add(buildPersonalWeaponRagRecord(items[i]));
    onProgress?.call(i + 1, items.length);
  }

  final version = await PersonalWeaponRepo().getSelectedVersion() ?? 'unknown';
  ...
  final file = File('${dir.path}/personal_weapons_rag_$safeVersion.json');
